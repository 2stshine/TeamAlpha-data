-- dividend_event_frequency_ttm Gold implementation.
-- value = signal date에 알려지고 실제 적용된 직전 12개월 canonical 현금배당 사건 수
-- predicted_sign = +1, 따라서 rank 1은 사건 수가 가장 많은 종목이다.
-- adjusted_cash_amount는 인증된 canonical 사건의 유효성 필터에만 쓰며,
-- 배당액이나 가격은 신호값 계산에 사용하지 않는다.
WITH current_contract AS (
    ...
    SELECT
        ...
        u.as_of_date,
        u.signal_month,
        count(e.action_key)::double precision AS value
    FROM universe u
    ...
    GROUP BY u.asset_id, u.as_of_date, u.signal_month
), ranked AS (
    SELECT
        asset_id,
        as_of_date,
