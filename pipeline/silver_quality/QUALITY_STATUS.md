...
Silver 품질 게이트의 현재 상태 기록. 최근 전체 감사(`s3_domain_audit`) 결과를
기준으로 하며, 새 전체 감사를 돌릴 때마다 이 문서의 수치를 갱신한다.

- 기준: Bronze cutoff `f6d0af48…` (2026-07-24), 가격 6,749,929행 · 재무 2,195,327행
- 룰셋: **1.17.0**
- 최종 갱신: 2026-07-28
- 종합: **차단 실패 0, 파이프라인發 데이터 오류 0.** Warning은 전수 조사 완료된
  잔여 신호(실제 이벤트 또는 DART 원천 오류), Modified는 결정적 규칙 변환.
...
전수 조사 완료. **모두 정당한 값이며 파이프라인이 만든 데이터 오류가 아니다.**
