...
class _CertificationCursor:
    ...
    def fetchall(self):
        if "FROM dividend_event_resolution" not in self.last_sql:
            raise AssertionError(self.last_sql)
        return [(
            1, "DART_DISCLOSURE", "20260101000001",
            "krx_dividend_resolution_v2", date(2026, 1, 5),
            10.0, 10.0, date(2026, 1, 2),
            100.0, 100.0, 100.0, 100.0,
            1.0, 1.0, 1.0, "STABLE_PRICE_SCALE", False,
            None, None, 1.0, 1.0, True,
        )]

    def fetchone(self):
        if "BEFORE_LISTING_OR_EPISODE_START" in self.last_sql:
            return (0, 0)
        if "SELECT count(*) FROM dividend_event_resolution" in self.last_sql:
            return (self.summary.cash_action_count,)
        return (
...
def test_issuer_ex_dividend_action_also_invalidates_contract(monkeypatch):
    calls = []
    frame = _published_action_frame()
